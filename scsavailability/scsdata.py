...
class ScsData():
    ...
    def floor_shift_time_fa(self, shift=0):
        self.fa_processed = feat.floor_shift_time_fa(self.fa_processed, shift=shift)
        print('fa_processed update with time shift')
    
    def create_ptt_df(self, weights):
        self.df, fa_ptt = feat.create_PTT_df(self.fa_processed, self.at_processed, self.av_processed, weights=weights)
        return fa_ptt
