...
class DBTests:
    def test_read_query(self):
        ...
        coeff_path = r'./scsavailability/data/sql/coeff.sql'
        ...
        coeff = db.read_query(sql_conn=conn, query_path=coeff_path)

        at_cols = list(at.columns)
        av_cols = list(av.columns)
        fa_cols = list(fa.columns)
        coeff_cols = list(coeff.columns)
        ...
        coeff_cols_expected = ['ID', 'COEFFICIENT', 'PTT', 'TIMESTAMP']                    

        assert at_cols == at_cols_expected and av_cols == av_cols_expected and fa_cols == fa_cols_expected and coeff_cols == coeff_cols_expected
