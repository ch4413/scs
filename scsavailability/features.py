...
def pre_process_av(av):
    ...
    4. drops faults that happen in same location at same time (keeps fault with
    max duration) - if selected
    Note: function will need to be adapated to preprocess availability data
     from other months
    '''
    # Rename time column
...
def add_code(data):
    ...
    """
    scs = data.copy()
    scs['Asset Code'] = scs['Alert'].str.extract(r'(^[A-Z]{3}[0-9]{3}|[A-Z][0-9]{4}[A-Z]{3}[0-9]{3}|[A-Z]{3} [A-Z][0-9]{2})')
    scs['Asset Code'] = scs['Alert'].str.extract(r'([A-Z][0-9]{4}[A-zZ]{3}[0-9]{3})')
...
def add_tote_colour(scs_code):
    ...
    df_totes.loc[df_totes['PLC'].isin(['C17', 'C16', 'C15', 'C23']),
                 'Tote Colour'] = 'Blue'
    df_totes['Pick Station'] = df_totes['Alert'].str.extract(r'(PTT[0-9]{3})').fillna(False)
    df_totes.loc[(df_totes['Pick Station'] != False), 'Tote Colour'] = 'Both'
    df_totes['PLCN'] = df_totes['PLC'].str.extract(r'((?<=C)[0-9]{2})').fillna(0).astype('int')
...
def preprocess_faults(fa, remove_same_location_faults=True, remove_warnings=True, remove_door=True):
    ...
    fa.rename(columns={fa.columns[3]: 'timestamp', 'index': 'Alert ID'},
              inplace=True)
    ...
    # Mark PTTs
    fa.loc[~(fa['Pick Station'].isin([False])), 'Desk_edit'] = fa[~(fa['Pick Station'].isin([False]))]['Pick Station'].apply(lambda x: x[:-1])
...
def fault_select(data, modules, PTT='None'):
    ...
    # 3 Module can't be assigned if Loop == Quadrant
...
def weight_hours(df, weights=[1]):
    ...
    Parameters:
        df: input data frame
    weights: weights for hours with first element in array being weight for
    current hour, second the previous hour etc.
    '''
...
def create_PTT_df(fa_floor, at, av, weights=None):
    ...
    pick_stations = ['PTT011', 'PTT012', 'PTT021', 'PTT022', 'PTT031', 'PTT032',
                     'PTT041', 'PTT042', 'PTT051', 'PTT052', 'PTT071', 'PTT072',
                     'PTT081', 'PTT082', 'PTT091', 'PTT092', 'PTT101', 'PTT102',
                     'PTT111', 'PTT112', 'PTT121', 'PTT122', 'PTT131', 'PTT132',
                     'PTT141', 'PTT142', 'PTT151', 'PTT152', 'PTT171', 'PTT172',
                     'PTT181', 'PTT182', 'PTT191', 'PTT192', 'PTT201', 'PTT202']
    df_PTT = pd.DataFrame()
    fa_PTT = []
    for PTT in pick_stations:
        ...
        df = merge_av_fa_at(av_agg, at_df=at_agg, fa_df=fa_agg,
                            agg_level=agg_level)
